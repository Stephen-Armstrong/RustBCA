...
fn test_surface_binding_energy_barrier() {
    ...
    let cosz = 0.;
    let mut particle_1 = particle::Particle::new(
        mass, Z, E, Ec, Es, x, y, z, cosx, cosy, cosz, false, false, 0,
    );
    ...
    let mesh_2d_input = mesh::Mesh2DInput {
        ...
        simulation_boundary_points: vec![
            (0., 1.1 * thickness / 2.),
            (depth, 1.1 * thickness / 2.),
            (depth, -1.1 * thickness / 2.),
            (0., -1.1 * thickness / 2.),
            (0., 1.1 * thickness / 2.),
        ],
    ...
    let surface_binding_energy =
        material_1.actual_surface_binding_energy(&particle_1, particle_1.pos.x, particle_1.pos.y);
    assert!(approx_eq!(
        f64,
        surface_binding_energy / EV,
        (2. + 4.) / 2.,
        epsilon = 1E-12
    ));
...
fn test_triangle_distance_to() {
    let triangle_1 = mesh::Triangle2D::new((0., 2., 0., 2., 0., 0.));
    assert!(
        approx_eq!(f64, triangle_1.distance_to(-2., 0.), 2., epsilon = 1E-12),
        "{}",
        triangle_1.distance_to(-2., 0.)
    );

    assert!(
        approx_eq!(
            f64,
            triangle_1.distance_to(2., 2.),
            (2.0_f64).sqrt(),
            epsilon = 1E-12
        ),
        "{}",
    ...
    assert!(
        approx_eq!(f64, triangle_1.distance_to(0., 0.), 0., epsilon = 1E-12),
        "{}",
        triangle_1.distance_to(0., 0.)
    );
    assert!(
        approx_eq!(f64, triangle_1.distance_to(2., 0.), 0., epsilon = 1E-12),
        "{}",
        triangle_1.distance_to(2., 0.)
    );
    assert!(
        approx_eq!(f64, triangle_1.distance_to(0., 2.), 0., epsilon = 1E-12),
        "{}",
        triangle_1.distance_to(0., 2.)
    );
}
...
fn test_surface_refraction() {
    ...
    let cosx_new =
        ((particle_1.E * particle_1.dir.x * particle_1.dir.x - Es) / (particle_1.E - Es)).sqrt();
    ...
    for energy_eV in vec![1., 10., 100., 1000., 1000.] {
        ...
        for high_energy_free_flight_paths in vec![true, false] {
            for potential in vec![
                ...
                for scattering_integral in vec![
                    ScatteringIntegral::MENDENHALL_WELLER,
                    ScatteringIntegral::GAUSS_MEHLER { n_points: 10 },
                    ScatteringIntegral::GAUSS_LEGENDRE,
                ] {
                    for root_finder in vec![Rootfinder::NEWTON {
                        ...
                        println!(
                            "Case: {} {} {} {}",
                            energy_eV,
                            high_energy_free_flight_paths,
                            potential,
                            scattering_integral
                        );

                        let mut particle_1 = particle::Particle::new(
                            m1, Z1, E1, Ec1, Es1, x1, y1, z1, cosx, cosy, cosz, false, false, 0,
                        );
                        ...
                        let binary_collision_geometries = bca::determine_mfp_phi_impact_parameter(
                            &mut particle_1,
                            &material_1,
                            &options,
                        );
                        ...
                        println!(
                            "Phi: {} rad p: {} Angstrom mfp: {} Angstrom",
                        ...
                        let binary_collision_result = bca::calculate_binary_collision(
                            &particle_1,
                            &particle_2,
                            &binary_collision_geometries[0],
                            &options,
                        ...
                        println!(
                            "E_recoil: {} eV Psi: {} rad Psi_recoil: {} rad",
                            binary_collision_result.recoil_energy / EV,
                            binary_collision_result.psi,
                            binary_collision_result.psi_recoil
                        );

                        println!(
                            "Initial Energies: {} eV {} eV",
                            particle_1.E / EV,
                            particle_2.E / EV
                        );
                        ...
                        particle_2.E = binary_collision_result.recoil_energy
                            - material_1
                                .average_bulk_binding_energy(particle_2.pos.x, particle_2.pos.y);
                        ...
                        println!(
                            "Final Energies: {} eV {} eV",
                            particle_1.E / EV,
                            particle_2.E / EV
                        );
                        println!(
                            "X: {} {} {}% Error",
                            initial_momentum.x / ANGSTROM / AMU,
                            final_momentum.x / ANGSTROM / AMU,
                            100. * (final_momentum.x - initial_momentum.x)
                                / initial_momentum.magnitude()
                        );
                        println!(
                            "Y: {} {} {}% Error",
                            initial_momentum.y / ANGSTROM / AMU,
                            final_momentum.y / ANGSTROM / AMU,
                            100. * (final_momentum.y - initial_momentum.y)
                                / initial_momentum.magnitude()
                        );
                        println!(
                            "Z: {} {} {}% Error",
                            initial_momentum.z / ANGSTROM / AMU,
                            final_momentum.z / ANGSTROM / AMU,
                            100. * (final_momentum.z - initial_momentum.z)
                                / initial_momentum.magnitude()
                        );
                        println!();
...
fn test_rotate_particle() {
    ...
    assert!(
        approx_eq!(f64, particle.dir.x, 0., epsilon = 1E-12),
        "particle.dir.x: {} Should be ~0.",
        particle.dir.x
    );
    assert!(
        approx_eq!(f64, particle.dir.y, 1., epsilon = 1E-12),
        "particle.dir.y: {} Should be ~1.",
        particle.dir.y
    );
    ...
    particle::rotate_particle(&mut particle, -psi, phi);
    assert!(
        approx_eq!(f64, particle.dir.x, cosx, epsilon = 1E-12),
        "particle.dir.x: {} Should be ~{}",
        particle.dir.x,
        cosx
    );
    assert!(
        approx_eq!(f64, particle.dir.y, cosy, epsilon = 1E-12),
        "particle.dir.y: {} Should be ~{}",
        particle.dir.y,
        cosy
    );
    ...
    particle::rotate_particle(&mut particle, psi, phi);
    assert!(
        approx_eq!(f64, particle.dir.x, 1., epsilon = 1E-12),
        "particle.dir.x: {} Should be ~1.",
        particle.dir.x
    );
    assert!(
        approx_eq!(f64, particle.dir.y, 0., epsilon = 1E-12),
        "particle.dir.y: {} Should be ~0.",
        particle.dir.y
    );
    ...
    assert!(
        approx_eq!(
            f64,
            particle.dir.x.powf(2.) + particle.dir.y.powf(2.) + particle.dir.z.powf(2.),
            1.
        ),
        "Particle direction not normalized."
    );
}
...
fn test_quadrature() {
    ...
    let options = Options {
        ...
        num_chunks: 1,
        use_hdf5: false,
        root_finder: vec![vec![Rootfinder::NEWTON {
            max_iterations: 100,
            tolerance: 1E-14,
        }]],
        track_displacements: false,
    ...
    let x0_newton = bca::newton_rootfinder(
        Za,
        Zb,
        Ma,
        Mb,
    ...
    if let Ok(x0_cpr) = bca::cpr_rootfinder(
        Za,
        Zb,
        Ma,
        Mb,
    ...
    let theta_gl =
        bca::gauss_legendre(Za, Zb, Ma, Mb, E0, p, x0_newton, InteractionPotential::KR_C);
    let theta_mw =
        bca::mendenhall_weller(Za, Zb, Ma, Mb, E0, p, x0_newton, InteractionPotential::KR_C);
    ...
    println!(
        "Gauss-Mehler: {} Gauss-Legendre: {} Mendenhall-Weller: {} MAGIC: {}",
