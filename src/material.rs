...
impl Material {
    pub fn new(
        ...
        let energy_unit: f64 = match material_parameters.energy_unit.as_str() {
            ...
            "MEV" => EV * 1E6,
            _ => panic!(
                "Input error: incorrect unit {} in input file. Choose one of: EV, J, KEV, MEV",
        ...
        let mass_unit: f64 = match material_parameters.mass_unit.as_str() {
            ...
            "KG" => 1.,
            _ => panic!(
                "Input error: incorrect unit {} in input file. Choose one of: AMU, KG",
        ...
        Material {
            ...
            m: material_parameters
                .m
                .iter()
                .map(|&i| i * mass_unit)
                .collect(),
            Z: material_parameters.Z,
            Eb: material_parameters
                .Eb
                .iter()
                .map(|&i| i * energy_unit)
                .collect(),
            Es: material_parameters
                .Es
                .iter()
                .map(|&i| i * energy_unit)
                .collect(),
            Ec: material_parameters
                .Ec
                .iter()
                .map(|&i| i * energy_unit)
                .collect(),
            interaction_index: material_parameters.interaction_index,
            electronic_stopping_correction_factor: material_parameters
    ...
    pub fn actual_surface_binding_energy(
        &self,
    ...
    pub fn choose(&self, x: f64, y: f64) -> (usize, f64, f64, f64, f64, usize) {
        ...
        for (component_index, cumulative_concentration) in
            cumulative_concentrations.iter().enumerate()
        {
            if random_number < *cumulative_concentration {
                return (
                    component_index,
                    self.Z[component_index],
                    self.m[component_index],
                    self.Ec[component_index],
                    self.Es[component_index],
                    self.interaction_index[component_index],
                );
            }
        }
    ...
    pub fn electronic_stopping_cross_sections(
        &self,
        particle_1: &super::particle::Particle,
        ...
        for (n, Zb) in self.n.iter().zip(&self.Z) {
            ...
            let S_low = LINDHARD_SCHARFF_PREFACTOR * (Za.powf(7. / 6.) * Zb)
                / (Za.powf(2. / 3.) + Zb.powf(2. / 3.)).powf(3. / 2.)
                * (E / Q / Ma * AMU).sqrt();
...
pub fn surface_binding_energy(particle_1: &mut particle::Particle, material: &material::Material) {
    ...
    if entering {
        ...
        //particle_1.add_trajectory();
            ...
            panic!(
                "Numerical error: surface boundary algorithm encountered an error. Check geometry."
            );
        }
    }
...
pub fn boundary_condition_2D_planar(
